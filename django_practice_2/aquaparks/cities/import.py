from cities.models import City, AquaPark

city_1 = City.objects.get(name="New-York")
city_2 = City.objects.get(name="Miami")
city_3 = City.objects.get(name="Boston")
city_4 = City.objects.get(name="Tampa")
city_5 = City.objects.get(name="Los-Angeles")

aquapark_1 = AquaPark.objects.get(id="1")
aquapark_2 = AquaPark.objects.get(id="2")
aquapark_3 = AquaPark.objects.get(id="3")
aquapark_4 = AquaPark.objects.get(id="4")
aquapark_5 = AquaPark.objects.get(id="5")
aquapark_6 = AquaPark.objects.get(id="6")
aquapark_7 = AquaPark.objects.get(id="7")
aquapark_8 = AquaPark.objects.get(id="8")
aquapark_9 = AquaPark.objects.get(id="9")
aquapark_10 = AquaPark.objects.get(id="10")